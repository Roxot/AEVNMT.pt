...
class Constraint(torch.nn.Module):
    def __init__(self, bound, relation, name=None,
                 multiplier_act=F.softplus, alpha=0., start_val=0.):
        """
        ...
        Source: https://github.com/eelcovdw/pytorch-constrained-opt
        ...
        if self.relation == 'eq' and multiplier_act is not None:
            sys.stderr.write(
                "WARNING using an activation that maps to R+ with an equality \
                 constraint turns it into an inequality constraint. Set multiplier_act=None \
                 when using eq."
            )
...
class MDRConstraint(Constraint):
    def __init__(self, min_kl, name="MDR", multiplier_act=F.softplus, alpha=0., start_val=0.):
        """
        MDR constraint, for optimizing an ELBO s.t. KL >= min_kl.
        """
        super().__init__(
            min_kl, 'ge', name=name,
            multiplier_act=multiplier_act,
            alpha=alpha, start_val=start_val
        )
...
class Wrapper:
    ...
    def __repr__(self):
        return 'Wrapper(' + self.obj.__repr__() + ')'
